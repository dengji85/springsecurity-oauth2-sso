...
import com.monkeyk.sos.config.CookieUtil;
 ...
 * @since 3.0
 */
public class HttpSessionRequestCacheaa implements RequestCache {
	...
	public void saveRequest(HttpServletRequest request, HttpServletResponse response) {
		if (requestMatcher.matches(request)) {
			...
			MyJwtTokenStore.map.put(CookieUtil.getCookieByName(request, "JSESSIONID"), savedRequest);
	...
	public SavedRequest getRequest(HttpServletRequest currentRequest,
		...
		String s = CookieUtil.getCookieByName(currentRequest, "JSESSIONID");
		if(null!=s) {
		return (SavedRequest)  MyJwtTokenStore.map.get(s);
		}
		/*if (session != null) {
	...
	public void removeRequest(HttpServletRequest currentRequest,
		...
		if (session != null) {
			...
			MyJwtTokenStore.map.remove(CookieUtil.getCookieByName(currentRequest, "JSESSIONID"));
		}
	}
